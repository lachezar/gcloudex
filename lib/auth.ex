defmodule GCloudex.Auth do
  ...
  def get_token_storage(type) do
    ...
    case type do
      :read_only ->
        {:ok, get_token_response} = GoogleAuth.for_scope(@storage_scope_read_only)

        get_token_response |> Map.get(:token)

      :read_write ->
        {:ok, get_token_response} = GoogleAuth.for_scope(@storage_scope_read_write)

        get_token_response |> Map.get(:token)

      :full_control ->
        {:ok, get_token_response} = GoogleAuth.for_scope(@storage_scope_full_control)

        get_token_response |> Map.get(:token)

      :sql_admin ->
        {:ok, get_token_response} = GoogleAuth.for_scope(@sql_scope_admin)

        get_token_response |> Map.get(:token)

      :cs_read_only ->
        {:ok, get_token_response} = GoogleAuth.for_scope(@cloud_scope_cs_read_only)
      ...
      :compute_read_only ->
        {:ok, get_token_response} = GoogleAuth.for_scope(@compute_read_only)

        get_token_response |> Map.get(:token)

      :compute ->
        {:ok, get_token_response} = GoogleAuth.for_scope(@compute)
